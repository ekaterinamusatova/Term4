    ...
    internal static class Program
    {
        ...
        static void Main()
        {
            ...
            mainForm.AddFilter(new PixelFilter<LighteningParameters>(
                ...
                (pixel, parameters) =>
                    new Pixel(
                        Math.Max(0, Math.Min(1, parameters.Coefficient * (pixel.R - 0.5) + 0.5)),
                        Math.Max(0, Math.Min(1, parameters.Coefficient * (pixel.G - 0.5) + 0.5)),
                        Math.Max(0, Math.Min(1, parameters.Coefficient * (pixel.B - 0.5) + 0.5)))));
            ...
            mainForm.AddFilter(new TransformFilter<MoveParameters>(
            "Сдвиг вверх",
            new MoveTransformer()
            ));
